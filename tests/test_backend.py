...
def test_api_backend_satisfies_the_protocol() -> None:
    http = HttpClient(
        subdomain="example",
...
def test_isinstance_check_proves_method_names_only_not_signatures() -> None:
...
def test_get_ticket_returns_the_raw_envelope() -> None:
...
def test_fake_backend_does_not_leak_its_stored_dict_by_reference() -> None:
...
def test_fake_backend_does_not_leak_a_nested_field_by_reference() -> None:
...
def test_fake_backend_raises_the_same_error_type_as_the_real_one_for_a_missing_id() -> None:
    with pytest.raises(exc.NotFound):
        FakeBackend().get_ticket(ticket_id=999)


def test_api_backend_calls_the_documented_path() -> None:
...
def test_api_backend_returns_the_envelope_unshaped() -> None:
...
def test_fake_and_api_backend_are_mutually_consistent_on_envelope_shape() -> None:
