...
| B16 | **Scoping triage (B0c)** — bucket every family now / later / never / blocked, with reasons. Inputs: the OpenAPI specs, `analysis/PRIOR-ART.md`, the official Zendesk MCP server (#17), and the official SDKs (A1). | open | Added 2026-09-17. The `never` bucket is the new part — it is the only control that survives a gate failure, because an operation that was never generated cannot be enabled by misconfiguration. Closes A1. |
