...
| C8 | **`assert_subject_permitted` hardcodes the literal `"ticket_id"` as the subject key.** `update_trigger` needs `trigger_id` and has no backing `Backend` method yet, so it currently fails closed by omission (no `ticket_id` in kwargs) rather than by a real subject-key concept. | open | Task 5 minor, deferred; restated by Task 6. Needs a per-tool subject-key concept (e.g. a `subject_key` field on `ToolSpec`) before admin-domain tools ship. |
...
| C11 | **`tests/test_public_api.py` hard-asserts the module count (`== 9`), so every new module needs the count bumped.** The count guards against a vacuous pass if `iter_modules` ever returns empty, but `>= 9` would keep that guard without the churn. | open | Task 3 minor, deferred. Same defect class as **E10**. |
